...
if [ "$grub" == "1" ]
	then
		if [ "$cpuvendor" == "AMD" ]
			then
				sed -i s/"quiet splash"/"amd_iommu=on iommu=pt kvm_amd.npt=1 kvm_amd.avic=1 pcie_acs_override=downstream,multifunction"/g /etc/default/grub
				update-grub
		...
		then
			if [ "$cpuvendor" == "AMD" ]
				then
					echo "Add "amd_iommu=on iommu=pt kvm_amd.npt=1 kvm_amd.avic=1 pcie_acs_override=downstream,multifunction" to GRUB_CMDLINE_LINUX_Default"
